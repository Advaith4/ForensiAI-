...
const evidenceTypes = [
  ...
  { key: "images", label: "Images", icon: FileScan, accept: ".jpg,.jpeg,.png" },
...
export default function App() {
  ...
  async function loadCase(caseId: string) {
    try {
      ...
      setSelectedCase({ ...mockReport, case_id: caseId });
    }
  }
  ...
  async function createAndUpload() {
    ...
    try {
      ...
      try {
        ...
        created = {
          case_id: `CASE-DEMO-${Date.now().toString().slice(-4)}`,
          victim_name: form.victim_name || "Demo Victim",
          incident_location: form.incident_location || "Demo Location",
          incident_date: form.incident_date,
          notes: form.notes,
          status: "processing",
          risk_level: form.priority,
          risk_score: form.priority === "HIGH" ? 88 : 54
        };
      }
      ...
      for (const item of evidenceTypes) {
        ...
        if (!file) continue;
        try {
          await uploadEvidence(created.case_id, item.key === "images" ? "metadata" : item.key, file, (value) => {
      ...
      try {
        ...
        // Demo stays functional even if backend analysis is unavailable.
      }
      await runLogs();
      ...
      for (let i = 0; i < 15; i++) {
        try {
          const res = await fetch(`http://127.0.0.1:8000/cases/${created.case_id}/results`);
      ...
      try {
        if (isBackendComplete) {
          report = await getReport(created.case_id);
        } else {
          throw new Error("Backend not complete");
        }
      } catch {
        report = { ...mockReport, case_id: created.case_id, victim_name: created.victim_name, incident_location: created.incident_location, incident_date: created.incident_date };
      }
